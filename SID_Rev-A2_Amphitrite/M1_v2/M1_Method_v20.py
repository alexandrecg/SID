...
class Method_1:
    ...
    def calc_phi(self, alpha):
        return 2/(2+(np.tan(alpha))**2)
        
    
    ##### Métodos - Injetor Real #####
    
    ## Calcula o valor do nº de Reynolds nos canais tangenciais do injetor
    def calc_reynolds_in(self,m_in,mi,n,r_in_orf):
        return 2*m_in/(mi*n*np.pi*r_in_orf)
     
    
    ## Calcula o Valor do Coeficiente de Preenchimento corrigido (Phi_eq)
    def calc_phi_eq(self, r_in_pos, r_n, n, r_in_orf, friction_coef):
        self.ak = r_in_pos*r_n/(n*r_in_orf**2+friction_coef*r_in_pos*(r_in_pos-r_n)/2)
        self.coefs = [self.ak, -2, 4, -2]
