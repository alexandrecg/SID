...
Alpha2 = 2*180/np.pi*np.arcsin(2*Mu_i*A_i/((1+np.sqrt(1-Phi_i))*np.sqrt(1-0*Mu_i**2*A_i**2/C**2)))
...
W = m_i*1e-3/(N_in*(np.pi*r_in**2)*1e-6*Rho_p) #Tangential Velocity



#Número de Reynolds nos canais tangenciais
Re_in = Rho_p*W*d_in*1e-3/Din_visc_p
...
Mu_w = Phi_w**1.5/(np.sqrt(2-Phi_w))


#Espessura de camada líquida real
t_as_w = R_n*(1-np.sqrt(1-Phi_w)) #mm
...
Ksi_in = -1*Alpha_in/150 + 1.1#-0.0204*(d_bx/d_in)**4-0.026*(d_bx/d_in)**3+0.3587*(d_bx/d_in)**2-0.6995*(d_bx/d_in)+0.5002
...
#Vazão mássica real
m_r = np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P0*1e6)/np.sqrt((2-Phi_w)/Phi_w**3+Ksi*A_i**2/R_in**2)*1e3#np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P0*1e6)/np.sqrt(1/Phi_w**2+A_w**2/(1-Phi_w)+(Ksi*N_in)*A_i**2/C**2)*1000

#coeficiente de descarga total
Mu_t = Mu_w/(np.sqrt(1+Mu_w**2*Ksi*A_i**2/C**2))

print("Velocidade tangencial (W): \t\t%.2f \tm/s"%(W))
print("Reynolds (Re_in): \t\t\t%.2E"%(Re_in))
print("Resistência de superfície (Lambda): \t%.4f"%(Lambda))
print("Parâmetro Geométrico real (A_w): \t%.2f"%(A_w))
print("Coef. de área livre real (Phi_w): \t%.4f"%(Phi_w))
...
print("Coeficinete de descarga total (Mu_t): \t%.4f"%(Mu_t))
print("Vazão mássica real (m_r): \t\t%.1f \tg/s"%(m_r))

##### Cálulo do ângulo de spray real #####

Alpha2 = 2*180/np.pi*np.arcsin(2*Mu_i*A_w/((1+np.sqrt(1-Phi_i))*np.sqrt(1-Ksi*Mu_i**2*A_i**2/C**2)))
...
I = 23.7/((R_in/R_n)**2.7*N_in**1.34*Phi_w**1.1*(L_cv/D_cv)**0.15)
...
for Delta_P in dPressure:
    ...
    Mu_w = Phi_w*np.sqrt(Phi_w)/(np.sqrt(2-Phi_w))
    ...
    #Vazão mássica real
    m_r = np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P*1e6)/np.sqrt((2-Phi_w)/Phi_w**3+Ksi*A_i**2/R_in**2)*1e3#np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P*1e6)/np.sqrt(1/Phi_w**2+A_i**2/(1-Phi_w)+(Ksi*N_in)*A_i**2/C**2)*1000
    ...
    Alfa_seq = 2*180/np.pi*np.arcsin(2*Mu_t*A_i*K/(1+np.sqrt(1+Phi_w)*np.sqrt(1-Ksi*Mu_t**2*A_i**2)/C**2)) if -1 <= 2*Mu_t*A_i*K/(1+np.sqrt(1+Phi_w)*np.sqrt(1-Ksi*Mu_t**2*A_i**2)/C**2) <= 1 else 0
