...
if(Fluid == "F"):
    ...
    Rho_p = 810 #800.0 #kg/m³
    
    Din_visc_p = 1.447*1e-3 #0.001095 #Pa.s
    ...
    Din_visc_p = 2.21e-3#7.64e-6 #Pa.s
...
Alpha2 = 2*180/np.pi*np.arcsin(2*Mu_w*A_i*K/((1+np.sqrt(1-Phi_eq))*np.sqrt(1-Ksi*Mu_i**2*A_i**2/C**2)))
...
for Delta_P in dPressure:
    ...
    if(t_as_i <= 0):
        print("ERROR: invalid t_as value (%.2E < 2600) [\u0394P = %.2f MPa]"%(Re_in,Delta_P))
        
    elif(t_as_i <= 0.1):
        print("Caution! Low value of t_as (%.2E < 2600) [\u0394P = %.2f MPa]"%(Re_in,Delta_P))
    ...
    elif(Re_in > 5e4): print("Caution: Reynolds number over 5e4 (%.2E < 2600) [\u0394P = %.2f MPa]"%(Re_in,Delta_P))
    ...
    #Vazão mássica real
    m_r = Mu_i*np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P*1e6)*1000 #np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P*1e6)/np.sqrt((2-Phi_eq)/Phi_eq**3+Ksi*A_i**2/R_in**2)*1e3
    #m_r = Mu_i*np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P*1e6)*1e3
    ...
    Alfa_seq = 2*180/np.pi*np.arcsin(2*Mu_w*A_eq/((1+np.sqrt(1-Phi_eq))*np.sqrt(1-Ksi*Mu_i**2*A_i**2/C**2))) #2*180/np.pi*np.arcsin(2*Mu_i*A_eq/((1+np.sqrt(1-Phi_eq))*np.sqrt(1-Ksi*Mu_i**2*A_i**2/C**2)))#2*180/np.pi*np.arcsin(2*Mu_i*A_i*K/(1+np.sqrt(1+Phi_eq)*np.sqrt(1-Ksi*Mu_i**2*A_i**2)/C**2)) if -1 <= 2*Mu_i*A_i*K/(1+np.sqrt(1+Phi_eq)*np.sqrt(1-Ksi*Mu_i**2*A_i**2)/C**2) <= 1 else 0
