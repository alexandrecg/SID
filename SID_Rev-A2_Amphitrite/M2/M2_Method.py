...
class Method_2:
    ...
    def run_M2(self):
        ...
        self.Mu_eq = self.Phi_r**1.5/(np.sqrt(2-self.Phi_r))
        self.Mu_r = self.Mu_eq/(np.sqrt(1+self.Mu_eq**2*self.Ksi*self.A_i**2/self.C**2))        
        
        #Vazão mássica real
        self.m_r = np.pi*(self.fr.d_n_1/2*1e-3)**2*np.sqrt(2*self.fr.Rho_1*self.fr.Delta_P0_1*1e5)/np.sqrt((2-self.Phi_r)/self.Phi_r**3+self.Ksi*self.A_i**2/(self.fr.R_0_1)**2)*1e3   #np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P0*1e5)/np.sqrt(1/Phi_r**2+A_r**2/(1-Phi_r)+(Ksi*N_in)*A_i**2/C**2)*1000
        ...
        print("Parâmetro Geométrico real (A_r): \t\t%.2f"%(self.A_r))
        print("Coef. de área livre real (Phi_r): \t\t%.4f"%(self.Phi_r))
        print("Espessura da camada líquida (t_as_r): \t%.4f \tmm"%(self.t_as_r))
        ...
        print("Coeficiente de descarga real (Mu_eq): \t%.4f"%(self.Mu_eq))
        print("Coeficinete de descarga total (Mu_r): \t%.4f"%(self.Mu_r))
        ...
        self.sin = 2*self.Mu_r*self.A_r / ((1+np.sqrt(1-self.Phi_i)) * np.sqrt(1-self.Ksi*self.Mu_r**2*self.A_i**2/self.C**2))
        self.Alpha2_r = 2*self.rad_to_deg * np.arcsin( self.sin ) if abs(self.sin)<=1 else 0
            
        print("Ângulo de Abertura do Spray (Alpha2_r): \t%.1f \tdeg"%(self.Alpha2_r))
        
        
        ##### Cálculo da não-uniformidade #####
        
        self.I = 23.7/((self.fr.R_0_1/(self.fr.d_n_1/2))**2.7*self.fr.N_1**1.34*self.Phi_r**1.1*(self.fr.L_s_1/self.fr.d_s_1)**0.15)
        ...
        for self.Delta_P in self.dPressure:
            ...
            #Vazão mássica real
            self.m_r = np.pi/4*(self.fr.d_n_1*1e-3)**2*np.sqrt(2*self.fr.Rho_1*self.Delta_P*1e5)/np.sqrt((2-self.Phi_r)/self.Phi_r**3+self.Ksi*self.A_i**2/self.fr.R_0_1**2)*1e3  #np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P*1e5)/np.sqrt(1/Phi_r**2+A_i**2/(1-Phi_r)+(Ksi*N_in)*A_i**2/C**2)*1000
            ...
            self.Alpha2_real.append(self.Alpha2_r)
        ...
        if(self.fr.Config == "Bi\n"):
            ...
            print("Ângulo de Abertura do Spray (Alpha2): \t%.1f \tdeg"%(self.Alpha2_i))
            ...
            #Parâmetro Geométrico equivalente
            self.A_r = self.calc_A(self.fr.R_0_2, (self.fr.d_n_2/2), self.fr.N_2, self.fr.d_in_2/2, self.Lambda) 
            ...
            #Espessura de camada líquida real
            self.t_as_r = self.fr.d_n_2/2*(1-np.sqrt(1-self.Phi_r)) #mm
            ...
            #Vazão mássica real
            self.m_r = np.pi/4*(self.fr.d_n_2*1e-3)**2*np.sqrt(2*self.fr.Rho_2*self.fr.Delta_P0_2*1e5)/np.sqrt((2-self.Phi_r)/self.Phi_r**3+self.Ksi*self.A_i**2/(self.fr.R_0_2)**2)*1e3   #np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P0*1e5)/np.sqrt(1/Phi_r**2+A_r**2/(1-Phi_r)+(Ksi*N_in)*A_i**2/C**2)*1000
            ...
            ##### ângulo do spray real #####
            self.sin = 2*self.Mu_r*self.A_r / ((1+np.sqrt(1-self.Phi_i)) * np.sqrt(1-self.Ksi*self.Mu_r**2*self.A_i**2/self.C**2))
            self.Alpha2_r = 2*self.rad_to_deg * np.arcsin( self.sin ) if abs(self.sin)<=1 else 0
                
            print("Ângulo de Abertura do Spray (Alpha2): \t%.1f \tdeg"%(self.Alpha2_r))
            
            #Ângulo do spray modificado
            self.a = 2*(1-self.Phi_r)**2/(2-self.Phi_r)
            self.Alpha2_out = 2*self.rad_to_deg*np.arctan(self.a/((self.fr.R_out_2/(self.fr.d_n_2/2))**2-self.a))
            
            #print("Ângulo de Abertura do Spray modificado (Alpha2_out): \t%.1f \tdeg"%(self.Alpha2_out))
            print("Ângulo de Abertura do Spray modificado (Alpha2_out): --- \tdeg")
            
            ##### Cálculo da não-uniformidade #####
            
            self.I = 23.7/((self.fr.R_0_2/(self.fr.d_n_2/2))**2.7*self.fr.N_2**1.34*self.Phi_r**1.1*(self.fr.L_s_2/self.fr.d_s_2)**0.15)
            ...
            for self.Delta_P in self.dPressure:
                ...
                #Vazão mássica real
                self.m_r = np.pi/4*(self.fr.d_n_2*1e-3)**2*np.sqrt(2*self.fr.Rho_2*self.Delta_P*1e5)/np.sqrt((2-self.Phi_r)/self.Phi_r**3+self.Ksi*self.A_i**2/self.fr.R_0_2**2)*1e3  #np.pi*(R_n*1e-3)**2*np.sqrt(2*Rho_p*Delta_P*1e5)/np.sqrt(1/Phi_r**2+A_i**2/(1-Phi_r)+(Ksi*N_in)*A_i**2/C**2)*1000
            
                #Ângulo de Spray
                self.sin = 2*self.Mu_r*self.A_r / ((1+np.sqrt(1-self.Phi_i)) * np.sqrt(1-self.Ksi*self.Mu_r**2*self.A_i**2/self.C**2))
                self.Alpha2_r = 2*self.rad_to_deg * np.arcsin( self.sin ) if abs(self.sin)<=1 else 0
                
                #Ângulo de Spray modificado
                self.a = 2*(1-self.Phi_r)**2/(2-self.Phi_r)
                self.Alpha2_out = 2*self.rad_to_deg*np.arctan(self.a/((self.fr.R_out_2/(self.fr.d_n_2/2))**2-self.a))
                #self.Phi_out = self.Phi_r/(3-2*self.Phi_r)
                #self.R_out_mn = self.fr.R_out_2*np.sqrt(1-self.Phi_out)
                #self.R_out_av = (self.R_out_mn+self.fr.R_out_2)/2
                ...
                self.Alpha2_mod.append(self.Alpha2_out)
            ...
            fig, ax21 = plt.subplots(figsize=(14, 7))
            ax21.grid()
            ...
            ax21.plot(self.fr.Delta_P0_2, self.fr.m_2, '.', markersize=20, label = "ṁxΔP alvo", color = '0.5')
            ax21.legend()
            ax21.set_ylabel("ṁ [g/s]")
            ax21.set_xlabel("ΔP [bar]")
            ax21.set_xticks(np.arange(0.0,10.0+0.2,0.2))
            self.y_min = round(min([min(self.Mass_ideal),min(self.Mass_real),min(self.M_target),self.fr.m_2]),0)
            self.y_max = round(max([max(self.Mass_ideal),max(self.Mass_real),max(self.M_target),self.fr.m_2]),0)
            self.y_gap = (self.y_max - self.y_min)/20
            ax21.set_yticks(np.arange(self.y_min,self.y_max+self.y_gap,self.y_gap))
            ax21.tick_params(axis='x', labelsize=8)
            ax21.tick_params(axis='y', labelsize=8)
            #ax21.set_yticklabels(fontdict=font)
            ...
            '''
            fig, ax24 = plt.subplots(figsize=(14, 7))
            ax24.grid()
            ...
            #ax24.plot(self.dPressure, self.Alpha2_mod, 'k-.', markersize=1, label = "2α modificado")
            ax24.legend()
            ax24.set_ylabel("2α [deg]")
            ax24.set_xlabel("ΔP [bar]")
            ax24.set_xticks(np.arange(0.0,10.0+0.2,0.2))
            self.y_min = round(min([min(self.Alpha2_ideal),min(self.Alpha2_real)]),1)
            self.y_max = round(max([max(self.Alpha2_ideal),max(self.Alpha2_real)]),1)
            self.y_gap = (self.y_max - self.y_min)/20
            ax24.set_yticks(np.arange(self.y_min,self.y_max+self.y_gap,self.y_gap))
            ...
            #ax25.grid()
            #ax25.set_title("ST2 - 2α_mod x ΔP") 
            #ax25.plot(self.dPressure, self.Alpha2_mod, 'k-.', markersize=1, label = "2α modificado")
            #ax25.legend()
