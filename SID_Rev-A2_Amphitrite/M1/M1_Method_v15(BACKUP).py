...
from M1_FileReader import FileReader
...
class Method():
    ...
    def executar():
        ...
        while erro_R >= erro_max and contador < self.it_lim:
            
            R_in_1 = self.Rs_Rn_1*R_n_1
            
            r_in_1 = np.sqrt(R_in_1*R_n_1/(self.n_1*A_1))
            
            entrance.append(r_in_1)
            
            L_in_1 = self.LD_in_1*r_in_1
            
            L_n_1 = self.LD_n_1*R_n_1
            
            L_s_1 = self.LD_s_1*R_in_1
            
            R_s_1 = R_in_1+r_in_1
        
            Re_in_1 = (2/np.pi)*self.m_1*1e-3/(n_1*r_in_1*1e-3*self.Din_visc_1)
            ...
            Mi_i_1 = Mi_eq_1/np.sqrt(1+Ksi_1*Mi_eq_1**2*A_1**2/self.Rs_Rn_1**2)
            
            Alpha2_eq_1_calc = 2*180/np.pi*np.arcsin(2*Mi_i_1*A_eq_1/((1+np.sqrt(1-Phi_eq_1))*np.sqrt(1-Ksi_1*Mi_i_1**2*A_1**2/self.Rs_Rn_1**2)))
            
            erro_R = R_n_1
            
            R_n_1 = 1e3/2* np.sqrt(4/np.pi) * np.sqrt(m_1*1e-3/(Mi_i_1*np.sqrt(2*Rho_1*self.Delta_P_1*1e5)))
            
            erro_R = abs(erro_R-R_n_1)
        ...
        fig, ax1 = plt.subplots()
        ax1.plot(range(contador),entrance)
        ax1.grid()
        ...
        print("Number of Inlet Channels = \t\t%.1f"%(self.n_1))
        ...
        if(Config == "Bi\n"):
            ...
            while contador < contador_max:
                ...
                R_in_2 = R_n_2-r_in_2#self.Rs_Rn_2*R_n_2
                
                r_in_2 = np.sqrt(R_in_2*R_n_2/(n_2*A_2))
                
                entrance.append(r_in_2)
                
                L_in_2 = LD_in_2*r_in_2
                ...
                Mi_i_2 = Mi_eq_2/np.sqrt(1+Ksi_2*Mi_eq_2**2*A_2**2/self.Rs_Rn_2**2)
                
                Alpha2_eq_2_calc = 2*180/np.pi*np.arcsin(2*Mi_i_2*A_eq_2/((1+np.sqrt(1-Phi_eq_2))*np.sqrt(1-Ksi_2*Mi_i_2**2*A_2**2/self.Rs_Rn_2**2)))
        ...
        ax3.set_ylim(-5.0,max(inj_H)+1.0)
        ...
        if(Config == "Bi\n"):
            ...
            ax4.set_ylim(-5.0,max(inj_H)+1.0)