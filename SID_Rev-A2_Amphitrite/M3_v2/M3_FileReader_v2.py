...
class FileReader_3:
    ...
    def read(self):
        ...
        self.r_in_orf_tol = float(self.DataStream[6])

        self.l_in = float(self.DataStream[7])
        self.l_in_tol = float(self.DataStream[8])
        
        self.inlet_radius = float(self.DataStream[9])
        self.inlet_radius_tol = float(self.DataStream[10])
        
        self.r_s = float(self.DataStream[11])
        self.r_s_tol = float(self.DataStream[12])
        
        self.l_s = float(self.DataStream[13])
        self.l_s_tol = float(self.DataStream[14])
        
        self.r_n = float(self.DataStream[15])
        self.r_n_tol = float(self.DataStream[16])
        
        self.l_n = float(self.DataStream[17])
        self.l_n_tol = float(self.DataStream[18])
        
        self.r_in_pos = float(self.DataStream[19])
        self.r_in_pos_tol = float(self.DataStream[20])
