...
class Method_2:
    def __init__(self, foldername, filename):
        ...
        self.delta_p_i = 1.00 #bar
        self.delta_p_f = 10.0 #bar
        self.delta_p_step = 0.01
        self.lambdaType = "Bazarov"
    ...
    def set_lambda_type(self,typeName):
        if(typeName == "Bazarov"):
            self.lambdaType = typeName
        elif(typeName == "Bayvel"):
            self.lambdaType = typeName
        else:
            print("ERROR: lambda type not understood")
    ...
    def calc_2alpha(self, phi):
        ...
        #OK###return 2*180/np.pi*np.arctan(np.sqrt(2*(1-coef)/coef))
    
    def calc_reynolds_in(self,m_in,din_visc,n,r_in_orf):
        return (2*m_in*1e-3)/(din_visc*n*np.pi*(r_in_orf*1e-3))
        #return (2*m_in)/(din_visc*np.sqrt(n)*np.pi*r_in_orf)
    
    def calc_friction_coef(self, reynolds_in):
            if(self.lambdaType == "Bazarov"):
                 return 0.3164*reynolds_in**(-0.25)          #Bazarov
            elif(self.lambdaType == "Bayvel"):
                return 10**(25.8/(np.log(reynolds_in)**2.58)-2)  #Bayvel
            else:
                print("ERROR: lambda type not understood")
    ...
    ## calcula o parâmetro geométrico característico corrigido (A_eq) [Bazarov, eq(99)]
    #def calc_geom_char_eq_alt(self, phi_eq):
        #return np.sqrt(2)*(1-phi_eq)/(phi_eq**1.5)
    
    ## Calcula o coeficiente de descarga equivalente [Bazarov, eq(99)]
    def calc_mu_eq(self, phi_eq):
        return phi_eq**1.5/np.sqrt(2-phi_eq)
        #OK###return phi_eq**1.5/np.sqrt(2-phi_eq)
    ...
    def calc_mu_i(self, mu_eq, geom_char, r_in_pos, r_n, ksi):
        ...
        #OK###return mu_eq/np.sqrt(1+ksi*mu_eq**2*(geom_char/(r_in_pos/r_n))**2)
    ...
    def run_M2(self):
        ...
        for self.delta_p in self.pressure_range:
            ...
            while self.erro_m >= self.erro_max and self.contador < self.it_lim:
                ...
                #Vazão mássica real
                self.m_i = np.pi*(self.fr.r_n*1e-3)**2*np.sqrt(2*self.fr.rho*self.delta_p*1e5)/np.sqrt((2-self.phi_eq)/self.phi_eq**3+self.ksi*self.A**2/(self.fr.r_in_pos/self.fr.r_n)**2)*1e3
                ...
                self.alpha2_i = self.calc_2alpha_avg(self.mu_i, self.A_eq, self.phi_eq, self.ksi, self.C)
            ...
            self.alpha2_real_eq.append(self.alpha2_eq)
            self.alpha2_real_i.append(self.alpha2_i)
            ...
            #if(self.rey_in < 2600): print("\nERROR: Low Reynolds number (%.2E < 2600) (delta_p = %.2f bar)"%(self.rey_in, self.delta_p))
            #elif(self.rey_in > 5e4): print("\nCaution: High Reynolds number (%.2E > 5e4) (delta_p = %.2f bar)"%(self.rey_in, self.delta_p))
            #if(self.rey_in < 1e4): print("\nERROR: Low Reynolds number (%.2E < 1e4) (delta_p = %.2f bar)"%(self.rey_in, self.delta_p))
            #elif(self.rey_in > 1e6): print("\nCaution: High Reynolds number (%.2E > 1e6) (delta_p = %.2f bar)"%(self.rey_in, self.delta_p))
                
            if(self.rey_in < 1e3): print("\nERROR: Low Reynolds number (%.2E < 1e4) (delta_p = %.2f bar)"%(self.rey_in, self.delta_p))
            elif(self.rey_in > 1e7): print("\nCaution: High Reynolds number (%.2E > 1e6) (delta_p = %.2f bar)"%(self.rey_in, self.delta_p))
        ...
        #ax11.grid()
        ax11.set_title("ṁ x ΔP", fontsize = font_size) 
        ax11.plot(self.pressure_range, self.m_ideal, 'k--', markersize=1, label = "ṁ fluido ideal")
        ...
        ax11.legend(fontsize = font_size)
        ax11.set_ylabel("ṁ [g/s]" ,fontsize = font_size)
        ax11.set_xlabel("ΔP [bar]", fontsize = font_size)
        ...
        #ax11.grid()
        ax11.xaxis.set_tick_params(labelsize=font_size)
        ax11.yaxis.set_tick_params(labelsize=font_size)
        '''
        ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### #####    
        ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### #####
        ##### ##### ##### ##### Plot Alpha2 x DeltaP ## ##### ##### ##### #####
        ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### #####
        ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### #####
        
        fig, ax11 = plt.subplots(figsize=(14, 7))
        #ax11.grid()
        ax11.set_title("2α x ΔP") 
        #ax11.plot(self.pressure_range, self.alpha2_ideal, 'k--', markersize=1, label = "2α fluido ideal", color = '0.5')
        #ax11.plot(self.pressure_range, self.alpha2_real_eq, 'k:', markersize=3, label = "2α fluido viscoso")
        ax11.plot(self.pressure_range, self.alpha2_real_i, 'k-.', markersize=3, label = "2α fluido viscoso")
        #ax11.plot(self.pressure_range, [self.fr.m_0]*len(self.pressure_range), 'k-.', markersize=1, label = "2α alvo")
        #ax11.plot(self.fr.delta_p0, self.fr.m_0, '.', markersize=20, label = "2αxΔP alvo", color = '0.5')
        ax11.legend()
        ax11.set_ylabel("2α [deg]")
